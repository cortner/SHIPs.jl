...
function SHIPBasis(Deg::AbstractDegree,
                   trans::DistanceTransform,
                   fcut::PolyCutoff)
   J = TransformedJacobi(maxK(Deg), trans, fcut)
   return SHIPBasis(Deg, J)
end

function SHIPBasis(Deg::AbstractDegree, J::TransformedJacobi)
   # R̂ - basis
   SH = SHBasis(maxL(Deg))
   # precompute the Clebsch-Gordan coefficients
   cg = ClebschGordan(maxL(Deg))
   # get the basis specification
   allKL, Nu = generate_KL_tuples(Deg, cg; filter=true)
